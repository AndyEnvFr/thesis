...
# specTIme of Schroeder run

# S <-getSpecTime(runsRaw$ndd1var20cut5_pdd1var5cut1, plot = FALSE)
# 
# pdf(paste0(root, "local/figures/plot_sr-params/SpecTimeSchroed.pdf"), width = 8, height = 5, onefile = TRUE, useDingbats = FALSE)
# plot(S$spec_rich ~ S$year, type = "l", xlab = "generations", ylab = "species richness")
# dev.off()
```
...
### same plot but all images merged
...
for (i in seq_len(nrow(combos))) {
  ...
  p <- p +
    ...
    theme(legend.position = "none",
          ...
          axis.title.y = element_text(size = 20, angle = 90),  # Increase y-axis label size
          axis.text = element_text(size = 18),  # Increase axis text size
          plot.title = element_text(size = 18, hjust = 0.5, vjust = -1.5)) +
...
plots_list[[16]] <- ggplot() +
  theme_minimal()

library(ggpubr)

# Create 3x3 plot grid
p1 <- ggarrange(plots_list[[1]], plots_list[[2]], plots_list[[3]], 
                plots_list[[4]], plots_list[[5]], plots_list[[6]], 
                plots_list[[7]], plots_list[[8]], plots_list[[9]],
                plots_list[[10]], plots_list[[11]], plots_list[[12]],
                plots_list[[13]], plots_list[[14]], plots_list[[15]],
                ncol = 3, nrow = 5)
print(p1)

# pdf(paste0(root, "local/figures/plot_sr-params/nddSlope.pdf"), width = 18, height = 26, onefile = TRUE, useDingbats = FALSE)
# print(p1)
# dev.off()
```
...
datmetaRan <- readRDS(paste0(root, "local/runs/plot_sr-params/metafor/datmetaRan.rds"))
metamodRan <- readRDS(paste0(root, "local/runs/plot_sr-params/metafor/metamodRan.rds"))
```
...
```{r}
pDDSR_ <- getSpecTime(pDDS, plot = FALSE)
nDDSR_ <- getSpecTime(nDDS, plot = FALSE)

names(pDDSR_) <- names(metamodPDD)
names(nDDSR_) <- names(metamodNDD)

pDDSR <- pDDSR_[["P1-L20-C5_N1-L5-C1"]] %>% 
  mutate(meanSR = mean(spec_rich),
         sdSR = sd (spec_rich)) %>% 
  dplyr::select(-year, -spec_rich) %>% 
  slice(1)
nDDSR <- nDDSR_[["P1-L5-C1_N1-L20-C5"]] %>% 
  mutate(meanSR = mean(spec_rich),
         sdSR = sd (spec_rich)) %>% 
  dplyr::select(-year, -spec_rich) %>% 
  slice(1)

nullSR <- nDDSR_[["P1-L5-C1_N0-L20-C1"]] %>% 
  mutate(meanSR = mean(spec_rich),
         sdSR = sd(spec_rich)) %>% 
  dplyr::select(-year, -spec_rich) %>% 
  slice(1)
...
## show relationship between Error and species abundance

```{r}
# schroeder sceanrio

# pdf(paste0(root, "local/figures/plot_sr-params/.pdf"), width = 6, height = 6, onefile = TRUE, useDingbats = FALSE)
# plot( metamodNDD$`P1-L5-C1_N1-L20-C5`$data$vi ~ metamodNDD$`P1-L5-C1_N1-L20-C5`$data$abund,
#      ylab = "variance after AME sampling", xlab = "species abundance")
# dev.off()

```

## with null and neutral settings
...
## --- Extract null model (N=0 scenario) ---
null_data <- pred_df_ndd_raw[pred_df_ndd_raw$N == 0 & pred_df_ndd_raw$NC == 1, ]
# Add treatment label for null
null_data$treatment <- "null"
...
combined_data$treatment <- factor(combined_data$treatment,
                                   levels = c("high pDD", "high nDD"))

x_range <- range(combined_data$abund[combined_data$abund > 0], na.rm = TRUE)

ppp <- ggplot() +
  # Add null model first (as background) - now with aes mapping
  geom_ribbon(data = null_data,
              aes(x = abund, ymin = ci_lower, ymax = ci_upper, fill = treatment),
              alpha = 0.2) +
  geom_line(data = null_data,
            aes(x = abund, y = fitted, color = treatment, linetype = treatment),
            size = 0.5, alpha = 0.9) +
  # Add treatment ribbons and lines
  geom_ribbon(data = ndd_data,
              aes(x = abund, ymin = ci_lower, ymax = ci_upper, fill = treatment),
              alpha = 0.2) +
  geom_line(data = ndd_data,
            aes(x = abund, y = fitted, color = treatment, linetype = treatment),
            size = 1.2) +
  scale_x_log10(limits = x_range, expand = expansion(mult = 0.02)) +
  scale_color_manual(values = c("high nDD" = "firebrick", "null" = "black")) +
  scale_fill_manual(values = c("high nDD" = "firebrick", "null" = "black")) +
  scale_linetype_manual(values = c("high nDD" = "solid", "null" = "dashed")) +
  labs(title = "",
       x = "species abundance",
       y = "stabilizing CDD (%)",
       color = "",
  ...
  theme(legend.position = "bottom",
        plot.title = element_text(size = 14, hjust = 0.5)) +
  coord_cartesian(ylim = c(-0.0005, 0.0010)) +
  guides(color = guide_legend(reverse = FALSE, override.aes = list(linewidth = 1)),
         fill = guide_legend(reverse = FALSE),
         linetype = guide_legend(reverse = FALSE)) +
...
```
```{r}
# pdf(paste0(root, "local/figures/plot_sr-params/paramSchroedNull.pdf"), width = 6, height = 6, onefile = TRUE, useDingbats = FALSE)
# print(ppp)
# dev.off()
```


## thrre randmozation plots
...
pred_ran <- lapply(datmetaRan, function(x){
  expand_grid(log_N = seq(min(x$log_N, na.rm = TRUE),
                          max(x$log_N, na.rm = TRUE),
                          length.out = 1000))
})
pred_ran <- lapply(pred_ran, function(x){
  x$abund <- exp(x$log_N)
  x
})
pred_ran <- lapply(seq_along(pred_ran), function(i){
  x <- pred_ran[[i]]
  y <- metamodRan[[i]]
  cbind(x, predict(object = y, newmods = x$log_N))
})
names(pred_ran) <- names(metamodRan)

pred_df_ran_raw <- do.call(rbind, lapply(seq_along(pred_ran), function(i) {
  df <- pred_ran[[i]]
  pars <- extract_params(names(pred_ran)[i])
  df$scenario <- names(pred_ran)[i]
  df$P  <- pars$P
  df$PL <- pars$PL
  df$PC <- pars$PC
  df$N  <- pars$N
...
if ("pred" %in% names(pred_df_ran_raw))   names(pred_df_ran_raw)[names(pred_df_ran_raw)=="pred"]   <- "fitted"
if ("fit"  %in% names(pred_df_ran_raw))   names(pred_df_ran_raw)[names(pred_df_ran_raw)=="fit"]    <- "fitted"
if ("ci.lb" %in% names(pred_df_ran_raw))  pred_df_ran_raw$ci_lower <- pred_df_ran_raw$ci.lb
if ("ci.ub" %in% names(pred_df_ran_raw))  pred_df_ran_raw$ci_upper <- pred_df_ran_raw$ci.ub
# --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
...
ranMort <- pred_df_ran_raw[pred_df_ran_raw$scenario == "ranMort", ]
ranCon <- pred_df_ran_raw[pred_df_ran_raw$scenario == "ranCon", ]
ranBoth <- pred_df_ran_raw[pred_df_ran_raw$scenario == "ranBoth", ]

# Combine data and add labels
neut_data$treatment <- "neutral run"
ndd_data$treatment <- "Schroeder scenario"
ranMort$treatment <- "randomized mortalities"
ranCon$treatment <- "randomized conspecifics"
ranBoth$treatment <- "both randomized"

combined_data <- rbind(
  neut_data[, c("abund", "fitted", "ci_lower", "ci_upper", "treatment")],
  ndd_data[, c("abund", "fitted", "ci_lower", "ci_upper", "treatment")],
  ranMort[, c("abund", "fitted", "ci_lower", "ci_upper", "treatment")],
  ranCon[, c("abund", "fitted", "ci_lower", "ci_upper", "treatment")],
  ranBoth[, c("abund", "fitted", "ci_lower", "ci_upper", "treatment")]
)

combined_data$treatment <- factor(combined_data$treatment,
                                   levels = c("neutral run",
                                             "Schroeder scenario",
                                             "randomized mortalities",
                                             "randomized conspecifics",
                                             "both randomized"
                                             ))
```
...
# =====================================================
# --- Plot 1: Randomized Mortalities ---
# =====================================================
combined_data_mort <- rbind(
  ranMort[, c("abund", "fitted", "ci_lower", "ci_upper", "treatment")],
  neut_data[, c("abund", "fitted", "ci_lower", "ci_upper", "treatment")],
  ndd_data[, c("abund", "fitted", "ci_lower", "ci_upper", "treatment")]
)
...
combined_data_mort$treatment <- factor(
  ifelse(combined_data_mort$treatment == "randomized mortalities", "randomized run",
  ifelse(combined_data_mort$treatment == "neutral run", "neutral run", "Schroeder scenario")),
  levels = c("randomized run", "neutral run", "Schroeder scenario")
)

x_range_mort <- range(combined_data_mort$abund[combined_data_mort$abund > 0], na.rm = TRUE)

plot1 <- ggplot(combined_data_mort, aes(x = abund, y = fitted,
                                     color = treatment,
                                     fill = treatment,
                                     linetype = treatment)) +
  geom_ribbon(aes(ymin = ci_lower, ymax = ci_upper), alpha = 0.2, color = NA) +
  geom_line(size = 1.2) +
  scale_x_log10(limits = x_range_mort, expand = expansion(mult = 0.02)) +
  scale_color_manual(values = c("#1E90FF", "darkgrey", "firebrick")) +
  scale_fill_manual(values = c("#1E90FF", "darkgrey", "firebrick")) +
  scale_linetype_manual(values = c("dashed", "dashed", "solid")) +
  labs(title = "Randomized Mortalities",
       x = "species abundance",
       y = "stabilizing CDD (%)") +
  theme_classic(base_size = 16) +
  theme(legend.position = "bottom",
        legend.title = element_blank(),
        plot.title = element_text(size = 14, hjust = 0.5, vjust = -8)) +
  coord_cartesian(ylim = c(-0.0005, 0.0010)) +
  guides(color = guide_legend(reverse = TRUE),
         fill  = guide_legend(reverse = TRUE),
         linetype = guide_legend(reverse = TRUE))
...
# =====================================================
# --- Plot 2: Randomized Conspecific Frequency ---
# =====================================================
combined_data_con <- rbind(
  ranCon[, c("abund", "fitted", "ci_lower", "ci_upper", "treatment")],
  neut_data[, c("abund", "fitted", "ci_lower", "ci_upper", "treatment")],
  ndd_data[, c("abund", "fitted", "ci_lower", "ci_upper", "treatment")]
)
...
# Standardize treatment labels
combined_data_con$treatment <- factor(
  ifelse(combined_data_con$treatment == "randomized conspecifics", "randomized run",
  ifelse(combined_data_con$treatment == "neutral run", "neutral run", "Schroeder scenario")),
  levels = c("randomized run", "neutral run", "Schroeder scenario")
)

x_range_con <- range(combined_data_con$abund[combined_data_con$abund > 0], na.rm = TRUE)

plot2 <- ggplot(combined_data_con, aes(x = abund, y = fitted,
                                    color = treatment,
                                    fill = treatment,
                                    linetype = treatment)) +
  geom_ribbon(aes(ymin = ci_lower, ymax = ci_upper), alpha = 0.2, color = NA) +
  geom_line(size = 1.2) +
  scale_x_log10(limits = x_range_con, expand = expansion(mult = 0.02)) +
  scale_color_manual(values = c("#1E90FF", "darkgrey", "firebrick")) +
  scale_fill_manual(values = c("#1E90FF", "darkgrey", "firebrick")) +
  scale_linetype_manual(values = c("dashed", "dashed", "solid")) +
  labs(title = "Randomized Conspecifics",
       x = "species abundance",
       y = "stabilizing CDD (%)") +
  theme_classic(base_size = 16) +
  theme(legend.position = "bottom",
...
# =====================================================
# --- Plot 3: Both Randomized ---
# =====================================================
combined_data_both <- rbind(
  ranBoth[, c("abund", "fitted", "ci_lower", "ci_upper", "treatment")],
  neut_data[, c("abund", "fitted", "ci_lower", "ci_upper", "treatment")],
  ndd_data[, c("abund", "fitted", "ci_lower", "ci_upper", "treatment")]
)
...
# Standardize treatment labels
combined_data_both$treatment <- factor(
  ifelse(combined_data_both$treatment == "both randomized", "randomized run",
  ifelse(combined_data_both$treatment == "neutral run", "neutral run", "Schroeder scenario")),
  levels = c("randomized run", "neutral run", "Schroeder scenario")
)

x_range_both <- range(combined_data_both$abund[combined_data_both$abund > 0], na.rm = TRUE)

plot3 <- ggplot(combined_data_both, aes(x = abund, y = fitted,
                                     color = treatment,
                                     fill = treatment,
                                     linetype = treatment)) +
  geom_ribbon(aes(ymin = ci_lower, ymax = ci_upper), alpha = 0.2, color = NA) +
  geom_line(size = 1.2) +
  scale_x_log10(limits = x_range_both, expand = expansion(mult = 0.02)) +
  scale_color_manual(values = c("#1E90FF", "darkgrey", "firebrick")) +
  scale_fill_manual(values = c("#1E90FF", "darkgrey", "firebrick")) +
  scale_linetype_manual(values = c("dashed", "dashed", "solid")) +
  labs(title = "Both Randomized",
       x = "species abundance",
       y = "stabilizing CDD (%)") +
  theme_classic(base_size = 16) +
  theme(legend.position = "bottom",
...
# =====================================================
# --- Combine plots in 3x1 layout ---
# =====================================================
pN <- ggarrange(plot1, plot2, plot3,
                ncol = 3, nrow = 1,
                common.legend = TRUE,
                legend = "bottom")
print(pN)
```
```{r}
# pdf(paste0(root, "local/figures/plot_sr-params/randomization.pdf"), width = 15, height = 5, onefile = TRUE, useDingbats = FALSE)
# print(pN)
# dev.off()
```
...
```{r}
# pdf(paste0(root, "local/figures/plot_sr-params/nddEffectOnMCV2.pdf"), width = 10.23, height = 5, onefile = TRUE, useDingbats = FALSE)
# print(plotNDD_MC)
# dev.off()
```
