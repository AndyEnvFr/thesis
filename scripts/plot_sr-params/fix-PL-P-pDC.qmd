...
# Plot
plotNDD <- ggplot(Sp2_final, aes(x = N, y = meanSR, color = factor(NL), group = NL)) +
  geom_line(size = .8) +
...
# saveRDS(tab, paste0(root, "local/runs/plot_sr-params/tab/fix-P-PL-nDC.rds"))
```

```{r}
tab <- readRDS(paste0(root, "local/runs/plot_sr-params/tab/fix-P-PL-nDC.rds"))
```
