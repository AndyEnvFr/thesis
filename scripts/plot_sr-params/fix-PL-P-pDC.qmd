...
# dat_meta <- readRDS(paste0(root, "local/runs/plot_sr-params/metafor/fix-PL-P-pDC_datmeta.rds"))
```
...
if (null_nameC1 %in% pred_df_raw$scenario) {
  null_dataC1 <- pred_df_raw[pred_df_raw$scenario == null_nameC1, ]
} 
#---
null_nameC3 <- "P1-L5-C1_N0-L20-C3"
if (null_nameC3 %in% pred_df_raw$scenario) {
  null_dataC3 <- pred_df_raw[pred_df_raw$scenario == null_nameC3, ]
}
#---
null_nameC5 <- "P1-L5-C1_N0-L20-C5"
if (null_nameC5 %in% pred_df_raw$scenario) {
  null_dataC5 <- pred_df_raw[pred_df_raw$scenario == null_nameC5, ]
} 
...
for (i in seq_len(nrow(combos))) {
  ...
  # set y-range depending on C
  if (c_i == 1) {
    p <- p + coord_cartesian(ylim = c(-0.0004, 0.0008))
  } else if (c_i == 3) {
    p <- p + coord_cartesian(ylim = c(-0.0004, 0.0008))
  } else if (c_i == 5) {
    p <- p + coord_cartesian(ylim = c(-0.0004, 0.0008))
...
plotNDD_MC <- ggplot() +
  ...
  geom_point(data = complete_data, 
             ...
             size = 2, position = position_dodge(width = 0.05)) +
  geom_errorbar(data = complete_data,
                aes(x = N, y = meanMC, color = factor(NL),
