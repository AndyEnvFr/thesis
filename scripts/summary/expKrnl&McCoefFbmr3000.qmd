...
boot_mort_change <- function(x, B = 1000) {
  ...
  for (i in seq_len(B)) {
    ...
    # fm <- tryCatch(glm(mortNextGen ~ con + abund, family = binomial(), data = xb),
...
# boot_results_iix <- do.call(cbind, boot_results_ii) %>%  as.data.frame()

# saveRDS(boot_results_iix, "~/cyber_synch/local/summary/boot_results_ii_+abund.rds")
# saveRDS(boot_results_iix, "~/cyber_synch/local/summary/boot_results_ii.rds")
```

```{r}
boot_results_std <- readRDS(paste0(root, "local/summary/boot_results_xii.rds"))
boot_results_abundMod <- readRDS(paste0(root, "local/summary/boot_results_ii_+abund.rds"))
```
...
x <- 1:ncol(boot_results_std)
y <- as.numeric(boot_results_std["mort_change",])
ci_lower <- as.numeric(boot_results_std["ci_lower",])
ci_upper <- as.numeric(boot_results_std["ci_upper",])
...
x <- 1:ncol(boot_results_abundMod)
y <- as.numeric(boot_results_abundMod["mort_change",])
ci_lower <- as.numeric(boot_results_abundMod["ci_lower",])
ci_upper <- as.numeric(boot_results_abundMod["ci_upper",])
