...
runsRaw  <- readRDS(paste0(root, "/local/runs/mstr/20250807/runs_viii.rds"))
```
...
runs_viii <- lapply(runsRaw, function(x){
...
cl <- makeCluster(length(runs_viii))
clusterExport(cl, c("getMatToTab", "runs_viii"))
tab_viii <- parLapply(cl = cl, X = runs_viii, fun = function(x) getMatToTab(x, detailedParams = TRUE))

saveRDS(tab_viii, paste0(root, "local/runs/mstr/20250807/tab_viii.rds"))
```

We only keep important/varying parameters in the naming. Fixed params are excluded.

```{r}
tab_viii <- readRDS(paste0(root, "local/runs/mstr/20250807/tab_vii.rds"))
```

```{r}
namesShort <- names(tab_viii) %>%
...
tab_viiiS <- lapply(tab_viii, function(x){
  res <- x %>%
...
cl <- makeCluster(length(tab_viiiS))

mcS_err <- parLapply(cl, tab_viiiS, function(x) {
...
saveRDS(dat_meta, paste0(root, "local/runs/mstr/20250807/dat_metaviii.rds"))
saveRDS(metamod, paste0(root, "local/runs/mstr/20250807/metamodviii.rds"))
...
```{r}
pdf(paste0(root, "local/figures/mstr/20250807/metafor/corr_128Schroed.pdf"), width = 8, height = 6, onefile = TRUE, useDingbats = FALSE)
