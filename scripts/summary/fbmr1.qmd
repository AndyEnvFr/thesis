...
**Here, I check neutral cases. Why do I see a correlation in the neutral case ?** **I want to do the same as in Hülsmann et al 2024: randomization of tree status (alive or dead sample(0:1, 10, replace = T)) and randomzation of conspecific densities from no to all conspecific neighbors sample (0:4, 10, replace = T)**
...
runs_v <- readRDS(paste0(root, "/local/runs/mstr/20250807/runs_v.rds"))
```
...
cl <- makeCluster(length(runs_v))
clusterExport(cl, c("getMatToTab", "runs_v"))
tab_v <- parLapply(cl = cl, X = runs_v, fun = function(x) getMatToTab(x, detailedParams = TRUE))

# saveRDS(tab_v, paste0(root, "local/runs/mstr/20250807/tab_v.rds"))
```

We only keep important/varying parameters in the naming. Fixed params are excluded.

```{r}
tab_v <- readRDS(paste0(root, "local/runs/mstr/20250807/tab_v.rds"))
```

```{r}
namesShort <- names(tab_v) %>%
...
tab_vx <- lapply(tab_v, function(x){
  res <- x %>%
  filter(census %% 2 == 0,
...
NbyCen <- lapply(tab_vx, function(x){
  res <- x %>% 
    group_by(specId) %>%
...
tab_vxx <- lapply(seq_along(tab_vx), function(i){
  res <- tab_vx[[i]] %>%
...
tab_vS <- lapply(tab_vxx, function(x){
  res <- x %>%
    filter(abundCen > 30 & abundCen < 500) %>%  # need species with enough variation in con for the model to estimate an effect
    mutate(specIdCen = paste0(specId, "_", census),
           con = as.integer(con))# loop through this variable for species wise glm. For each combination of specId and Cen a sperate model is done.
...
cl <- makeCluster(length(tab_vS))

mcS_err <- parLapply(cl, tab_vS, function(x) {
...
plot(metamod$`P1-L1_N1-L20`)
plot(metamod$`P1-L1_N1-L1`)
```
...
null_data1 <- pred_df[pred_df$NL == 20, ]
null_data2 <- pred_df[pred_df$NL == 1, ]
...
# check if mortality is proportionally higher for rare species in neutral scenario

```{r}
tab <- tab_vS [[1]]
rip <- tab %>%
  group_by(specIdCen) %>% 
  mutate(rip = (sum(mortNextGen) / first(abundCen)))

fm1 <- lm(rip ~ con, data = rip)
summary(fm1)  
```
