...
runs_iii <- readRDS(paste0(root, "/local/runs/mstr/20250807/runs_iii.rds"))
```
...
# saveRDS(tab_iii, paste0(root, "local/runs/mstr/20250807/tab_iii.rds"))
```
...
pred <- lapply(dat_meta, function(x){
  expand_grid(log_NperCen = seq(min(x$log_NperCen, na.rm = TRUE),
                          max(x$log_NperCen, na.rm = TRUE),
                          length.out = 50))
})
pred <- lapply(pred, function(x){
...
pdf(paste0(root, "local/figures/mstr/20250807/metafor/OverwritingProcetion.pdf"), width = 8, height = 6, onefile = TRUE, useDingbats = FALSE)
