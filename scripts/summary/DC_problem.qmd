...
**Here, I show the problem of GLM if DC are different for pDD and nDD**

Runs are not comparable, if they have different DC!
...
Here comes two possible different methods:
    for RNULL getConNeigh sets radius as the maximum DC for nDD or pDD: e.g., if nDC = 1, pDC = 1, only von Neumann neighborhood. If nDC = 3, pDC = 1, it gets 28 circular neighbors.
```{r}
#| eval: false

# get conspecific neighbors and proper naming
runsRNULL  <- getConNeigh(runsRaw, radius = NULL)
```
...
sapply(mcS_err0_corrected, function(x){
  return(round(x$mort_change[1],8))
})
```
```{r}
...
While for the C3C3 run the corFac could compensate for the effects of different neihgborhoods, for the C3C1 run this was not possible, because the corFac corrects for both, nDD and pDD and they have different kernels here. 
...
cl <- makeCluster(length(tab3S))

mcS_err3 <- parLapply(cl, tab3S, function(x) {
  specIDs <- unique(x$specIdCen)
  res <- vector("list", length(specIDs))
...
cl <- makeCluster(length(mcS_err0))
mcS_err_sim0 <- parLapply(cl, mcS_err0, function(x){
  lapply(x, function(y){
...
cl <- makeCluster(length(mcS_err3))
mcS_err_sim3 <- parLapply(cl, mcS_err3, function(x){
  lapply(x, function(y){
    sim <- MASS::mvrnorm(n = 100, mu = c(y$coef[1], y$coef[2]), Sigma = y$vcov)
    mort0 <- plogis(sim[, 1])
    mort1 <- plogis(sim[, 1] + sim[, 2])
...
m40 <- lapply(mcS_err_sim0, function(group) {
  do.call(rbind, group)
})

m40 <- lapply(m40, function(group) {
  row.names(group) <- NULL
  group <- group %>%
    mutate(log_N = log(abund))
  return(group)
})
m43 <- lapply(mcS_err_sim3, function(group) {
  do.call(rbind, group)
})

m43 <- lapply(m43, function(group) {
  row.names(group) <- NULL
  group <- group %>%
    mutate(log_N = log(abund))
  return(group)
...
dat_meta0 <- lapply(m40, function(x) {
  escalc(measure = "GEN", yi = mean, sei = se, slab = specId, data = x)
})

dat_meta3 <- lapply(m43, function(x) {
  escalc(measure = "GEN", yi = mean, sei = se, slab = specId, data = x)
})
```
...
```{r}
cl <- makeCluster(length(dat_meta0))
clusterExport(cl, varlist = c("dat_meta0"), envir = environment())

metamod0 <- parLapply(cl, dat_meta0, function(x) {
  tryCatch({
    metafor::rma(
      yi = yi,
      vi = vi,
...
failed_indices <- sapply(metamod0, is.null)
namesShort <- names(metamod0)
metamod0 <- metamod0[!failed_indices]
names(metamod0) <- namesShort[!failed_indices]
...
```{r}
cl <- makeCluster(length(dat_meta3))
clusterExport(cl, varlist = c("dat_meta3"), envir = environment())

metamod3 <- parLapply(cl, dat_meta3, function(x) {
  tryCatch({
    metafor::rma(
      yi = yi,
      vi = vi,
...
# Remove NULL elements and keep names aligned
failed_indices <- sapply(metamod3, is.null)
namesShort <- names(metamod3)
metamod3 <- metamod3[!failed_indices]
names(metamod3) <- namesShort[!failed_indices]
...
# saveRDS(dat_meta0, paste0(root, "local/runs/mstr/20250807/CD_problem_datmeta0.rds"))
# saveRDS(dat_meta3, paste0(root, "local/runs/mstr/20250807/CD_problem_datmeta3.rds"))
# saveRDS(metamod0, paste0(root, "local/runs/mstr/20250807/CD_problem_metamod0.rds"))
# saveRDS(metamod3, paste0(root, "local/runs/mstr/20250807/CD_problem_metamod3.rds"))


D0<- readRDS(dat_meta0, paste0(root, "local/runs/mstr/20250807/CD_problem_datmeta0.rds"))
D3<- readRDS(dat_meta3, paste0(root, "local/runs/mstr/20250807/CD_problem_datmeta3.rds"))
M0<- readRDS(metamod0, paste0(root, "local/runs/mstr/20250807/CD_problem_metamod0.rds"))
M3<- readRDS(metamod3, paste0(root, "local/runs/mstr/20250807/CD_problem_metamod3.rds"))
```
...
pred <- lapply(dat_meta0, function(x){
...
```{r}
## --- build predictions (as you had) ---
pred <- lapply(dat_meta3, function(x){
  expand_grid(log_N = seq(min(x$log_N, na.rm = TRUE),
                          max(x$log_N, na.rm = TRUE),
                          length.out = 1000))
})
...
```{r}
datmeta <- readRDS(paste0(root, "local/runs/plot_sr-params/metafor/fix-N-NL-nDC_datmeta.rds"))
metamod <- readRDS(paste0(root, "local/runs/plot_sr-params/metafor/fix-N-NL-nDC_metamod.rds"))
