...
**Also, I dont do a species wise model for the whole generation, but I do a model for group_by(species,census). I.e., if a species exists in two censii, i do two models and check correaltion between MC and abundance for two different abundances**

```{r}
tab_iii <- readRDS(paste0(root, "local/runs/mstr/20250807/tab_iii.rds"))
tabRan <- c(tab_iii, tab_iii[1], tab_iii[1], tab_iii[1])

namesShort <- c(names(tab_iii), "ranCon", "ranMort", "ranConMort") %>%
...
names(tabRan) <- namesShort
```

```{r}
tabRan$ranCon$con <- sample(x = 0:4, size = nrow(tabRan$ranCon), replace = TRUE)
tabRan$ranMort$mortNextGen <- sample(c(FALSE, TRUE), size = nrow(tabRan$ranMort), replace = TRUE)
tabRan$ranConMort$mortNextGen <- sample(c(FALSE, TRUE), size = nrow(tabRan$ranConMort), replace = TRUE)
tabRan$ranConMort$con <- sample(x = 0:4, size = nrow(tabRan$ranConMort), replace = TRUE)
```
...
tabRanX <- lapply(tabRan, function(x){
  res <- x %>%
  filter(census %% 2 == 0,
...
tabRanXX <- lapply(seq_along(tabRanX), function(i){
  res <- tabRanX[[i]] %>%
...
tabRanS <- lapply(tabRanXX, function(x){
  res <- x %>%
    filter(abundCen > 100) %>% # 30
    mutate(specIdCen = paste0(specId, "_", census)) # loop through this variable for species wise glm. For each combination of specId and Cen a sperate model is done.
  return(res)
})
```
...
cl <- makeCluster(length(tabRanS))

mcS_err <- parLapply(cl, tabRanS, function(x) {
...
# Run in parallel
metamodTot <- parLapply(cl, dat_meta, function(x) {
  metafor::rma(
    yi = yi,
    vi = vi,
   mods = ~ log_Ntot,
...
names(metamodTot) <- namesShort
```

```{r}
# saveRDS(metamodTot, paste0(root, "local/runs/mstr/20250807/metamodiiiTot250.rds"))
# saveRDS(dat_meta, paste0(root, "local/runs/mstr/20250807/dat_metaiiiSpecIdCen250.rds"))
metamod <- readRDS(paste0(root, "local/runs/mstr/20250807/metamodiiiTot250.rds"))
dat_meta <- readRDS(paste0(root, "local/runs/mstr/20250807/dat_metaiiiSpecIdCen250.rds"))
```
...
extract_params <- function(name) {
  # Handle special cases for ranCon and ranMort
  if (name %in% c("ranCon", "ranMort", "ranConMort")) {
    return(data.frame(P = NA, PL = NA, N = NA, NL = NA))
  }
...
create_plot_with_null <- function(data, title) {
  ggplot() +
    ...
    geom_line(data = data, aes(x = abund, y = fitted, color = factor(paste0("L", PL))), 
              size = 1) +
    scale_x_log10() +
    labs(title = title, x = "Abundance Tot", y = "Mortality Change",
...
p3 <- ggplot() +
  ...
  geom_line(data = p3_data, aes(x = abund, y = fitted, color = factor(paste0("L", NL))), 
            size = 1) +
  scale_x_log10() +
  labs(title = "P0, N0.5 - Varying N Lambda", x = "Abundance Tot", y = "Mortality change",
...
p4 <- ggplot() +
  ...
  geom_line(data = p4_data, aes(x = abund, y = fitted, color = factor(paste0("L", NL))), 
            size = 1) +
  scale_x_log10() +
  labs(title = "P0, N1 - Varying N Lambda", x = "Abundance Tot", y = "Mortality change",
       color = "N Lambda", fill = "N Lambda") +
  theme_minimal() +
  theme(legend.position = "bottom")
...
p5 <- ggplot() +
  ...
  labs(title = "N0.5, P0.5 - Varying N and P Lambda", 
       x = "Abundance Tot", y = "Mortality change",
...
# 7. Panel with null and the two new scenarios (ranCon and ranMort)
p7_data <- pred_df[pred_df$scenario %in% c("ranCon", "ranConMort", "ranMort"), ]
p7 <- ggplot() +
  # Add null reference line
  geom_ribbon(data = null_data, aes(x = abund, ymin = ci_lower, ymax = ci_upper), 
              alpha = 0.2, fill = "black") +
  geom_line(data = null_data, aes(x = abund, y = fitted), 
            color = "black", linetype = "dashed", size = 1.2) +
  # Add new scenario lines
  geom_ribbon(data = p7_data, aes(x = abund, ymin = ci_lower, ymax = ci_upper, 
                                 fill = scenario), alpha = 0.3) +
  geom_line(data = p7_data, aes(x = abund, y = fitted, color = scenario), size = 1) +
  scale_x_log10() +
  labs(title = "Random Control and Random Mortality Scenarios", 
       x = "Abundance Tot", y = "Mortality change",
...
p4 <- plots[[4]]; p5 <- plots[[5]]; p6 <- plots[[6]]; p7 <- plots[[7]]
...
# Run in parallel
metamodCen <- parLapply(cl, dat_meta, function(x) {
  metafor::rma(
    yi = yi,
    vi = vi,
   # mods = ~ log_Ntot,
...
names(metamodCen) <- namesShort
```

```{r}
# saveRDS(metamodCen, paste0(root, "local/runs/mstr/20250807/metamodiiiCen250.rds"))
...
# 7. Panel with null and the two new scenarios (ranCon and ranMort)
p7_data <- pred_df[pred_df$scenario %in% c("ranCon", "ranMort", "ranConMort"), ]
p7 <- ggplot() +
  # Add null reference line
  geom_ribbon(data = null_data, aes(x = abund, ymin = ci_lower, ymax = ci_upper), 
              alpha = 0.2, fill = "black") +
...
pdf(paste0(root, "local/figures/mstr/20250807/metafor/corr_abundCen250.pdf"), width = 8, height = 6, onefile = TRUE, useDingbats = FALSE)
