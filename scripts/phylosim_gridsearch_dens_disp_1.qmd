...
The implemented simulation batch function has no option to save interim results. 
Computation can still be parallelized in a safe way:
```{r}
set.seed(20250416)

results <- mclapply(1:length(params), function(x) {
  tryCatch({
    result <- runSimulation(params[[x]])
    save_out(sim_out = result,
             path = "~/", batch = FALSE, slim = TRUE)
  }, error = function(e) {
    message("Fehler in Job ", x, ": ", e$message)
    NULL
  })
}, mc.cores = 4)
```
