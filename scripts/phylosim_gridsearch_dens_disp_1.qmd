...
dispersal_options <- c(1, "global")
densityCut_options <- c(1,2,3,4,5)  # Values for densityCut
  ...
  for (dispersal in dispersal_options) {
    for (cut in densityCut_options) {
      ...
      params[[param_index]] <- createCompletePar(
        ...
        densityCut = cut,  # Now using the variable cut
        seed = 20250414,
        type = "base",
        protracted = 0,
        fission = 0,
        ...
        scenario = paste0("dd",
                        ...
                        scenario_dispersal,
                        "_sr2_e0_m1_cut",
                        scenario_cut)  # Updated scenario name to include cut value
      )
