...
This demo also demonstrated that finding the appropriate specificity term was not straightforward and involved a degree of subjective judgment. Further, there is a feedback between the chosen specificity term and the other DD settings. Thereby, I strongly oriented the implementation on the neutral scenario and the fitness values it produced. So, I tried to neither favor a high- nor a low phyllogenetic distance within the community. Eventually, increasing the responsiveness of the  feedbacks between DD effects and evolution towards the parameterization of the model, allowing communities with low- or high phyllogenetic distance. Another step to ensure such impartial behavior was to define the specificity such to have approximately similar integral near the range of neutral scenarios.

```{r}
areas <- sapply(lambda_values, function(lam) {
  integrate(function(x) kernel(x, lambda = lam, factor = 20), 
            lower = 0, upper = 0.05)$value # 0.05 comprises neutral scenario
})
paste0("lambda = ", lambda_values, " : area = ",round(areas,4))
```

Still, the feedback between evolution can favor high or low specificity values.
```{r}
# favoring high specificy value, if relatedness in the community is low:

lowRelatedness <- .02
highRelatedness <- .2

areasLow <- sapply(lambda_values, function(lam) {
  integrate(function(x) kernel(x, lambda = lam, factor = 20), 
            lower = 0, upper = lowRelatedness)$value # 0.05 comprises neutral scenario
})
cat("low relatedness\n: ",paste0("lambda = ", lambda_values, " : area = ",round(areasLow,4),"\n"), "\n\n")

areasHigh <- sapply(lambda_values, function(lam) {
  integrate(function(x) kernel(x, lambda = lam, factor = 20), 
            lower = 0, upper = highRelatedness)$value # 0.05 comprises neutral scenario
})
cat("high relatedness\n", paste0("lambda = ", lambda_values, " : area = ",round(areasHigh,4),"\n"), "\n\n")
```



The next demo uses a break-point approach to unveil fitness values that are produced inside PhyloSim at different settings.

# Illustration of fitness values in PhyloSim with the applied specificity kernel

Here, I apply the exponential kernel to PhyloSim and show the resulting fitness values for a standardized run (10x10 landscape for 5000 generations). The loaded files are [debug-files](https://github.com/AndyEnvFr/EcoPhyloSim/blob/53948a3913d225342f6325e65768940fc67d5e45/PhyloSim/src/Grid.cpp#L822-L837) from PhyloSim. In the next example, with pDD enabled, the base fitness of 1 is not reduced but exceeded.
