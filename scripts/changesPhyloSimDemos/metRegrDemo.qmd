...
-   for each run extract species wise information (conspecific neighbors and species abundance)
Now we have species wise information. The next steps are applied to each species of each run.
...
The strength is a simple scalar that ranges from 0 to 1.
For details on the specificity kernel see XXXXX.
In a first step, I present the spatial kernel (param = densityCut).
