...
The runs that I load are generated with a random sampling of the nDD and pDD strength, specificity kernel, and spatial kernel (see S4). The strength is a simple scalar that ranges from 0 to 1, the specificity kernel amplifies the DD effect for related neighbors and reduces it for unrelated (For details, see S1), and the spatial kernel (param = `densityCut`) dictates the range of effect of the DD effects. Latter is presented first.
...
From the coefficients and the variances-covariance matrix we simulated and averaged stabilizing CDD (Average Marginal Effect AME, see. Mood 2010.) with its standard error and 95% intervals (ci_low and ci_high).
...
metamod <- readRDS(paste0(root, "local/runs/mstr/20250903/ranMetamodMax_i.rds"))
dat_meta <- readRDS(paste0(root, "local/runs/mstr/20250903/ranDatmetaMax_i.rds"))
```
...
metafor::plot.rma.uni(metamod[[1]])
```
