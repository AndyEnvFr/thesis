...
# Define seed options - 5 different seeds by changing only the last digit
# The base part (20250602) remains consistent, and the distinguishing part is appended.
seed_options <- c(2025060201)
  ...
  for (dispersal_type in dispersal_options) {
    for (cut in densityCut_options) {
      for (fbmr in fitnessBaseMortalityRatio_options) {
        for (current_seed in seed_options) { # New loop for seeds
          ...
          # Extract the changing part of the seed for the scenario name
          # This assumes the last two digits are the 'changed number' as per request.
          ...
          params[[param_index]] <- createCompletePar(
            ...
            densityCut = cut,
            seed = current_seed, # Use the current seed from the loop
            type = "base",
            protracted = 0,
            fission = 0,
            redQueen = 0,
            ...
            runs = c(100,102,103),# c(sort( c( ((1:10 * 50)^2), ((1:10 * 50)^2) + 1 ))),
            scenario = paste0("dd", scenario_density,
                              "_disp", scenario_dispersal,
                              "_sr2_e0_m", scenario_fbmr,
                              "_cut", scenario_cut,
                              "_seed", scenario_seed_suffix) # Add the seed suffix to the scenario name
          )
