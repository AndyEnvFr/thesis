    ...
    for (cut in densityCut_options) {
      for (fbmr in fitnessBaseMortalityRatio_options) {
        ...
        params[[param_index]] <- createCompletePar(
          ...
          runs = sort(c(rep(1:5 * 100), rep(1:5 * 100) + 1, 1, 2)),
...
partest <- createCompletePar(
          ...
          airmat = 0,
          fitnessActsOn = "mortality",
          runs = sort(c((1:5 * 100), (1:5 * 100) + 1, 1, 2)),
          scenario = "test"
        )
```
...
```{r}
# runs <- runSimulationBatch(params[16:18], parallel = 3)
run <- runSimulation(partest)
```
