...
nddVar_options <- c(0.01, 0.1)
pddVar_options <- c(0.01, 0.1)
        ...
        for (env in environment_options) {  # << neue Schleife
          for (current_seed in seed_options) {
            ...
            params1[[param_index]] <- createCompletePar(
              ...
              runs = c(50,51,52)#c(sort( c( ((1:100 * 5)^2), ((1:100 * 5)^2) + 1 )))
            )
...
```{r}
for (i in 1:36) {
  run <- runSimulation(par = params1[[i]])
}

runz1 <- runSimulationBatch(params1, parallel = 36, backup = TRUE)
saveRDS(runz1, "~/cyber_synch/local/runs/fat/new/20250701_01")
rm(runz1)

runz2 <- runSimulationBatch(params2, parallel = 36, backup = TRUE)
saveRDS(runz2, "~/cyber_synch/local/runs/fat/new/20250701_02")
rm(runz2)

runz3 <- runSimulationBatch(params3, parallel = 36, backup = TRUE)
saveRDS(runz3, "~/cyber_synch/local/runs/fat/new/20250701_03")
rm(runz3)

runz4 <- runSimulationBatch(params4, parallel = 36, backup = TRUE)
saveRDS(runz4, "~/cyber_synch/local/runs/fat/new/20250701_04")
rm(runz4)

runz5 <- runSimulationBatch(params5, parallel = 36, backup = TRUE)
saveRDS(runz5, "~/cyber_synch/local/runs/fat/new/20250701_05")
rm(runz5)
