...
tab <- lapply(seq_along(tab), function(i){
  res <- tab[[i]] %>%
    group_by(specId) %>% 
    mutate(abund = n())  %>%
    left_join(NperCen[[i]], by = "specId") %>%
    mutate(NperCen = abund / n_census) %>%
    ungroup() %>%
    dplyr::select(census, specId, mortNextGen, con, abund, NperCen)
  return(res)
})
...
m4 <- lapply(m4, function(group) {
  ...
  group <- group %>% 
    mutate(log_NperCen = log(NperCen))
  return(group)
})
```

```{r}
# saveRDS(m4, paste0(root, "local/runs/mstr/20250725/metafor_interim/metafor_NperCen.rds"))
...
metamod <- parLapply(cl, dat_meta, function(x) {
  metafor::rma(yi = x$yi, vi = x$vi, mods = ~ x$log_NperCen, method = "REML", data = x)
})
...
pred <- lapply(dat_meta, function(x){
  expand_grid(log_NperCen = seq(min(x$log_N, na.rm = TRUE),
                          max(x$log_N, na.rm = TRUE),
                          length.out = 50))
})
pred <- lapply(pred, function(x){
  x$abund <- exp(x$log_NperCen)
  return(x)
})
...
pred <- lapply(seq_along(pred), function(i){
  ...
  return(cbind(x, predict(object = y, newmods = x$log_NperCen)))
})
```
...
```{r}
# # save some results
# saveRDS(pred, paste0(root, "local/runs/mstr/20250725/metafor_interim/pred_NperCen.rds"))
# saveRDS(metamod, paste0(root, "local/runs/mstr/20250725/metafor_interim/metamod_NperCen.rds"))
# saveRDS(dat_meta, paste0(root, "local/runs/mstr/20250725/metafor_interim/datmeta_NperCen.rds"))
...
group_plots <- lapply(names(mc_groups), function(group_name) {
  ...
  # Create the plot
  p <- ggplot(combined_data, aes(x = log_NperCen, y = pred, color = scenario, fill = scenario)) + 
    geom_ribbon(aes(ymin = ci.lb, ymax = ci.ub), alpha = 0.3) +
    geom_line(size = 1) +
    geom_hline(yintercept = 0, lty = 2) +
    labs(x = "Species Abundance\n(corrected for occurring censii)", y = "mortality\nchange", title = group_name) +
...
```{r}
# pdf(paste0(root, "/local/figures/mstr/20250725/metafor/NperCen.pdf"), width = 8, height = 6)
