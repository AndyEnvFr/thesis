...
saveRDS(tab_list, "~/cyber_synch/local/runs/tabular/20250527_mortality")
saveRDS(runtime, "~/cyber_synch/local/runs/tabular/20250527_mortality_runtime")
...
df <- tab_list[[1]] %>%
  mutate(het_con = as.factor(het_con), # case_when(het_con == "het" ~ TRUE, .default = FALSE),
         census = as.factor(census),
         mort_census_end = as.logical(mort_census_end)) %>%
           select(census, mort_census_end, het_con, starts_with("exp"))
...
run <- 1
for (dc in decay) {
  form <- as.formula(paste0("mort_census_end ~ s(het_con) +
                            s(", dc,") + s(census)"))
...
mod <- gam(mort_census_end ~ het_con + s(exp1, k = 1) + census,
